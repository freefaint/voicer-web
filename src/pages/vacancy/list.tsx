...
export class Vacancies extends React.Component<IProps, IState> {
  public state: IState = {};
    ...
    return (
      <>
          ...
          <ColumnSpace small={true}>
            <Row style={{ marginTop: "1rem" }}>
              <Column style={{ flexGrow: 1, marginRight: "1rem" }}>
                <Input autoFocus={true} name="msg" value={this.state.search} placeholder={"Поиск вакансий"} onChange={this.handleChange}></Input>
              </Column>
        ...
        {this.state.items && !!this.state.items.length && this.state.items.map(item => item.user && item.user._id !== (user && user._id) && (
          <Vacancy item={item} user={this.props.user} />
        ))}
      </>
    );
  };
  ...
  private handleLoad = () => {
    VacancyService.findItems({ name: this.state.search }).then(items => this.setState({ items }));
  };
}